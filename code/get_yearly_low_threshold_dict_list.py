...
pat_lon = []
pat_lat = []
size_thresholds = [0, 300]#, 100, 300]
n_Ks = [12]#[10, 11, 12, 13, 14, 15, 16]
...
for n_K in n_Ks:
    ...
    open_label = int(np.load(f"/uio/hume/student-u37/fslippe/data/models/patch_size128/filter{filter}/clustering/cluster_dnb_l95_z50_ps128_band29_filter{filter}_K{n_K}_opencell_label.npy"))
    closed_label = int(np.load(f"/uio/hume/student-u37/fslippe/data/models/patch_size128/filter{filter}/clustering/cluster_dnb_l95_z50_ps128_band29_filter{filter}_K{n_K}_closedcell_label.npy"))
    ...
    for size_threshold in size_thresholds:
        ...
        np.save(f"/uio/hume/student-u37/fslippe/data/model_pred_info/filter{filter}/dict_filter{filter}_nK{n_K}_caothr{threshold}_sizethr_{size_threshold}_stride{stride}_year{dict_list_year}", dict_list)
